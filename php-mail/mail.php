...
$Referer_check_domain = "xs365626.xsrv.jp";
...
$noticeSubject = "ホームページのお問い合わせがあります";
...
$require = array('お名前','Email','Email(確認用)','電話番号','お問い合わせ内容','個人情報の取り扱いについて');
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
	...
	//通知先に届くメールをセット
	$noticeBody = mailToNotice($_POST,$noticeSubject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp);
	$noticeHeader = noticeHeader($refrom_name,$from,$encode);
	$noticeSubject = "=?iso-2022-jp?B?".base64_encode(mb_convert_encoding($noticeSubject,"JIS",$encode))."?=";
	
	//-fオプションによるエンベロープFrom（Return-Path）の設定(safe_modeがOFFの場合かつ上記設定がONの場合のみ実施)
	if($use_envelope == 0){
		mail($to,$subject,$adminBody,$header);
		mail($notice_to,$noticeSubject,$noticeBody,$noticeHeader);
		if($remail == 1 && !empty($post_mail)) mail($post_mail,$re_subject,$userBody,$reheader);
	}else{
		mail($to,$subject,$adminBody,$header,'-f'.$from);
		mail($notice_to,$noticeSubject,$noticeBody,$noticeHeader,'-f'.$from);
...
<html lang="ja" dir="ltr">
	...
	<head>
		...
		<title>確認画面 | 事務局へお問い合わせ</title>
		<link rel="stylesheet" href="/assets/css/reset.css">
		<link rel="stylesheet" href="/assets/css/common.css">
		<link rel="stylesheet" href="/assets/css/contact_ck.css">
	...
	<body>
		<header>
			<div class="container">
			<div class="logo">
				<h1><a href="./"><img src="/assets/img/logo.png" alt="特定非営利活動法人全国認定こども園協会宮城県支部"></a></h1>
			</div>
			<div class="drawer only_sp">
				...
				<ul class="drawer_nav">
					...
					<li><a href="/training.html">研修会のご案内</a></li>
					<li><a href="/facility.html">施設紹介・求人情報</a></li>
					<li><a href="/kindergartens.html">加盟園について</a></li>
					...
					<div class="section_container">
						...
						<div id="formWrap">
							...
							<div class="error_wrap">
								...
								<div class="error_messe_wrap">
									<?php echo $errm; ?>
								</div>
								<input class="form_back_only" type="button" value="前画面に戻る" onClick="history.back()">
							</div>
							<?php }else{ ?>
							<p class="form_msg">以下の内容で間違いがなければ、「送信する」ボタンを押してください。</p>
							<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
								<table>
									<?php echo confirmOutput($_POST);//入力内容を表示?>
								</table>
								<p class="form_btn_wrap">
									<input type="hidden" name="mail_set" value="confirm_submit">
									<input type="hidden" name="httpReferer" value="<?php echo h($_SERVER['HTTP_REFERER']);?>">
									<input class="form_back" type="button" value="前画面に戻る" onClick="history.back()">
		...
		<footer id="footer">
			<div class="container">
				<ul class="footer_nav">
					<li><a href="#">組織概要</a></li>
					<li><a href="#">研修会のご案内</a></li>
					<li><a href="#">施設紹介・求人情報</a></li>
					<li><a href="#">加盟園について</a></li>
					<li><a href="#">プライバシーポリシー</a></li>
				</ul>
				<div class="footer_company">
					<a href="http://www.kodomoenkyokai.org/" class="footer_company_logo"><img src="/assets/img/footer_logo.png" alt="特定非営利活動法人全国認定こども園協会"></a>
					<div class="footer_company_text">「全国認定こども園協会」、および「認定こども園協会」の名称は、特定非営利活動法人全国認定こども園協会が商標登録しております。<br>商標登録番号5649503号</div>
				</div>
				<p class="footer_copy"><small>&copy;2020 全国認定こども園協会 宮城県支部</small></p>
			</div>
		</footer>
...
function noticeHeader($refrom_name,$to,$encode){
	$reheader = "From: ";
...
function mailToNotice($arr,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	$noticeBody="ホームページのお問い合わせがあります。\n";
	$noticeBody.="専用のメールアドレスを確認してください。\n";
	$noticeBody.="\n";
	/*
	$noticeBody.="送信された日時：".date( "Y/m/d (D) H:i:s", time() )."\n";
	$noticeBody.="送信者のIPアドレス：".@$_SERVER["REMOTE_ADDR"]."\n";
	$noticeBody.="送信者のホスト名：".getHostByAddr(getenv('REMOTE_ADDR'))."\n";
	*/
	if($confirmDsp != 1){
		$noticeBody.="問い合わせのページURL：".@$_SERVER['HTTP_REFERER']."\n";
	}else{
		$noticeBody.="問い合わせのページURL：".@$arr['httpReferer']."\n";
	}
	if($mailFooterDsp == 1) $noticeBody.= $mailSignature;
	return mb_convert_encoding($noticeBody,"JIS",$encode);
}
